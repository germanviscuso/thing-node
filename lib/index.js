...
module.exports = {
  ...
  onboardThingByUser(vendorThingId, thingPassword, user, callback) {
    const accessToken = user.getAccessToken();
    const userId = user.getID();
    const apiAuthor = this.getThingIFApiAuthor(accessToken);
    const owner = new _thingif.TypedID(_thingif.Types.User, userId);
    const request = new _thingif.OnboardWithVendorThingIDRequest(vendorThingId, thingPassword, owner);
  ...
  sendThingCommand(thingId, thingCommand, accessToken, callback) {
    const contentType = 'application/json';
    const baseUrl = thingifApp.getThingIFBaseUrl();
    const url = baseUrl + '/targets/thing:' + thingId + '/commands';
